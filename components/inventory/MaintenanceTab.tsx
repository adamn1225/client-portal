...
const MaintenanceTab = ({ freightList, maintenanceList, editFreight, handleDeleteClick, userId, setMaintenanceList }: MaintenanceTabProps) => {
    ...
    return (
        <div className="w-full bg-white dark:bg-zinc-800 dark:text-white shadow overflow-hidden rounded-md border border-zinc-400 max-h-max overflow-y-auto flex-grow">
            <div className="hidden xl:block overflow-x-auto">
                <table className="min-w-full divide-y parent-container divide-zinc-200 dark:divide-stone-100/50">
                    <thead className="bg-zinc-50 dark:bg-zinc-800 dark:text-white">
                        <tr className='border-b border-zinc-900/20 dark:border-zinc-100/20'>
                            <th className="dark:border-zinc-900 px-6 py-1 text-left text-nowrap text-xs font-normal dark:text-normal dark:font-medium uppercase tracking-wider border-r border-zinc-900/20  dark:border-zinc-100/20 ">Maintenance Item</th>
                            <th className="dark:border-zinc-900 px-6 py-1 text-left text-nowrap text-xs font-normal dark:text-normal dark:font-medium uppercase tracking-wider border-r border-zinc-900/20  dark:border-zinc-100/20 ">Parts Needed?</th>
                            <th className="dark:border-zinc-900 px-6 py-1 text-left text-nowrap text-xs font-normal dark:text-normal dark:font-medium uppercase tracking-wider border-r border-zinc-900/20  dark:border-zinc-100/20">Maintenance Notes</th>
                            ...
                            <th className="dark:border-zinc-900 px-6 py-1 text-left text-nowrap text-xs font-normal dark:text-normal dark:font-medium uppercase tracking-wider border-r border-zinc-900/20  dark:border-zinc-100/20">Assigned To</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-zinc-500  dark:text-white uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-zinc-200 dark:divide-white dark:bg-zinc-800 dark:text-white">
                        {maintenanceList.map((freight) => (
                            <tr key={freight.id}>
                                <td className="px-6 py-1 whitespace-nowrap border-r border-zinc-900/20  dark:border-zinc-100/20  dark:text-white">
                ...
                {maintenanceList.map((freight) => (
                    <div key={freight.id} className="bg-white dark:bg-zinc-800 dark:text-white shadow rounded-md mb-4 p-4 border border-zinc-400">
                        <div className="flex justify-between items-center mb-2">
                            <div className="text-sm font-extrabold text-zinc-500  dark:text-white">Inventory ID</div>
                            <div className="text-sm text-zinc-900">{freight.freight_id}</div>
                        </div>
                        <div>
                            <div className="text-sm font-extrabold text-zinc-500  dark:text-white">Commodity</div>
                            <div className="text-sm font-medium text-zinc-900">{freight.commodity || `${freight.year} ${freight.make} ${freight.model}`}</div>
                        </div>
                        <div>
                            <div className="text-sm font-extrabold text-zinc-500  dark:text-white ">Dimensions</div>
                            <div className="text-sm font-medium text-zinc-900">{freight.dimensions}</div>
                        </div>
                        <div className='border-b border-zinc-600 mb-4'></div>
                        <div className="flex flex-col justify-start items-stretch mb-2">
                            <div className="text-sm font-extrabold text-zinc-500  dark:text-white ">Urgency</div>
                            <div className="text-sm font-medium text-zinc-900">{freight.urgency}</div>
                        </div>
                        <div className="flex flex-col justify-start items-stretch mb-2">
                            <div className="text-sm font-extrabold text-zinc-500  dark:text-white ">Notes</div>
                            <div className="text-sm font-medium text-zinc-900">{freight.notes}</div>
                        </div>
                        <div className="flex flex-col justify-start items-stretch mb-2">
                            <div className="text-sm font-extrabold text-zinc-500  dark:text-white ">Maintenance Crew</div>
                            <div className="text-sm font-medium text-zinc-900">{freight.maintenance_crew}</div>
                        </div>
